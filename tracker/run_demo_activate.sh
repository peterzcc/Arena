...
script="python dqn_async.py -r roms/breakout.bin --replay-start-size 90 --optimizer rmspropnoncentered --lr 0.001 --eps 0.01 --symbol nips --sample-policy recent --rms-decay 0.99 --nactor 16 --single-batch-size 5 --param-update-period 5 --kvstore-update-period 16 --eps-update-period 1000 --kv-type dist_async --easgd-alpha 0.003"

python traker/dmlc_sge.py --activate-cmd ${activate_cmd} --log-file ${name}_n${nworker}_s${nserver}.out -wd ${wd} --jobname dmlc-${name}-n${n}-s${s} -workerq ${workerq} -serverq ${serverq} -c ${CTX} -n ${nworker} -s ${nserver} ${script}
