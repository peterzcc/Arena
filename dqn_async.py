...
class ActorLearnerThread(Thread):
    def __init__(self,pair,central_qnet=None,target_qnet=None,queue = None,args=None,use_easgd=None,
        ...
        self.rows = rows
        self.cols = cols
        self.eps_update_period = eps_update_period
    ...
    def run(self):
        ...
        states_buffer_for_act = numpy.zeros(
                                (1, self.history_length)+(self.rows, self.cols),dtype='uint8')
        states_buffer_for_train = numpy.zeros(
                            (self.single_batch_size, self.history_length+1)+(self.rows, self.cols),
                            dtype='uint8')
        self.game.start()
        self.game.begin_episode()
        ...
        while steps_left > 0:
            if local_steps % self.eps_update_period == 0:
                ...
                if eps_rand<0.4:
                    ...
                    eps_policy = 2
            if self.game.episode_terminate:
                episode += 1
                epoch_reward += self.game.episode_reward
                ...
                info_str += "Thread[%d], Epoch:%d, Episode:%d, Steps Left:%d/%d, Reward:%f, fps:%f, Exploration:%f" \
                            % (self.g,self.epoch, episode, steps_left, self.steps_per_epoch, self.game.episode_reward,
                               ave_fps, (eps_curr[eps_policy]))
                ...
                self.game.begin_episode(steps_left)
                episode_stat = EpisodeStat()

            if self.game.replay_memory.size > self.history_length:
                current_state = self.game.current_state()
                states = nd.array(current_state.reshape((1,) + current_state.shape),ctx=self.q_ctx) / float(255.0)
                qval_npy = self.qnet.forward(batch_size=1, data=states)[0].asnumpy()
                actions_that_max_q = numpy.argmax(qval_npy)
            # 1. We need to choose a new action based on the current game status
            if self.game.state_enabled and self.game.replay_memory.sample_enabled:
                do_exploration = (npy_rng.rand() < eps_curr[eps_policy])
                if do_exploration:
                ...
                action = npy_rng.randint(self.action_num)
            # for self.game,action in zip(games,actions):
            ...
            if total_steps % 100 ==0:
                eps_curr = numpy.maximum(eps_curr - self.eps_decay, self.eps_min)
                # 3. Update our Q network if we can start sampling from the replay memory
                #    Also, we update every `update_interval`
            if local_steps > self.single_batch_size and \
                local_steps % (self.param_update_period) == 0 and \
                self.game.replay_memory.sample_enabled:
                # 3.1 Draw sample from the replay_memory
                training_steps += 1

                # parallel_executor.map(sample_training_data,games,episode_stats,list(range(nactor)))
                episode_stat.episode_update_step += 1
                nsample = self.single_batch_size
                if self.args.sample_policy == "recent":
                    action, reward, terminate_flag=self.game.replay_memory.sample_last(batch_size=nsample,\
                        states=states_buffer_for_train,offset=0)
                elif self.args.sample_policy == "random":
                    action, reward, terminate_flag=self.game.replay_memory.sample_inplace(batch_size=nsample,\
                        states=states_buffer_for_train,offset=0)
                states = nd.array(states_buffer_for_train[:,:-1], ctx=self.q_ctx) / float(255.0)
                next_states = nd.array(states_buffer_for_train[:,1:], ctx=self.q_ctx) / float(255.0)
                actions = nd.array(action, ctx=self.q_ctx)
                rewards = nd.array(reward, ctx=self.q_ctx)
                terminate_flags = nd.array(terminate_flag, ctx=self.q_ctx)
                ...
                outputs = self.qnet.forward(batch_size=self.single_batch_size,is_train=True, data=states,
                                          dqn_action=actions,
...
def main():
    ...
    eps_decay = (eps_start - eps_min)*100 / (args.exploration_period)
    eps_curr = eps_start
    ...
    for epoch in xrange(epoch_num):
        ...
        central_qnet.save_params(dir_path=args.dir_path, epoch=epoch)
        if args.kv_type != None:
