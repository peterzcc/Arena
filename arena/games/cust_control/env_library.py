...
from arena.games.cust_control.const_dir_single_gather_env import ConstDirSingleGatherEnv
import os
...
hrl_reachreg = OrderedDict(reachreg=None,
                           **task_8_joint)
hrl_constdirreachreg = OrderedDict(constdirreachreg=None,
                                   **task_8_joint)
...
def make_env(env_name, withimg, T=1000, pid=0, initial_state_dir=None):
    ...
    if env_name == "custant":
        ...
        env = SingleGatherEnv(file_path=cwd + "/cust_ant.xml", with_state_task=False,
                              ...
                              subtask_dirs=subtask_dirs,
                              use_internal_reward=False)
    elif env_name == "flatdynamic2d5task8joint":
        env = SingleGatherEnv(file_path=cwd + "/cust_ant.xml", with_state_task=False,
                              f_gen_obj=random_direction8,
                              reset_goal_prob=0.005,
                              use_internal_reward=False)
        ...
        env = SingleGatherEnv(file_path=cwd + "/cust_ant.xml", with_state_task=False,
                              ...
                              obj_dist=1.25,
                              use_sparse_reward=True, )
    elif env_name == "reachc1task8joint":
        env = SingleGatherEnv(file_path=cwd + "/cust_ant.xml", with_state_task=False,
                              f_gen_obj=random_cont_direction,
                              catch_range=1,
                              obj_dist=1.25,
                              use_sparse_reward=True, )
    elif env_name == list(hrl_reachc05task8joint.keys())[0]:
        env = SingleGatherEnv(file_path=cwd + "/cust_ant.xml", with_state_task=False,
                              f_gen_obj=random_cont_direction,
                              catch_range=0.5,
                              obj_dist=1.25,
                              use_sparse_reward=True, )
    elif env_name == "flatreachc05task8joint":
        env = SingleGatherEnv(file_path=cwd + "/cust_ant.xml", with_state_task=False,
                              f_gen_obj=random_cont_direction,
                              catch_range=0.5,
                              obj_dist=1.25,
        ...
        env = ConstDirSingleGatherEnv(file_path=cwd + "/cust_ant.xml", with_state_task=False,
                                      ...
                                      goal_reward=5.0,
                                      regenerate_goals=True)
    elif env_name == list(hrl_constdirreach.keys())[0]:
        num_frame = 3
        subtask_dirs = np.stack([v() for (k, v) in list(task8.items())], axis=0)
        env = ConstDirSingleGatherEnv(file_path=cwd + "/cust_ant.xml", with_state_task=False,
                                      f_gen_obj=random_direction8,
