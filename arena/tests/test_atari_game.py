...
game = AtariGame(resize_mode='scale', replay_memory_size=2000, replay_start_size=replay_start_size, display_screen=False)
...
for i in xrange(totoal_time_step):
    ...
    if game.replay_memory.sample_enabled:
        ...
        sample_total_num += 1
        state_trajectory, action_trajectory, reward_trajectory, terminate_t = game.replay_memory.sample_trajectory(10)
        traj_len = state_trajectory.shape[0]
        print state_trajectory.shape
        print 'trajectory length is %d, with terminate flag %s' % (traj_len, terminate_t)
        # for ii in xrange(traj_len):
        #     plt.imshow(state_trajectory[ii,:,:], cmap=cm.Greys_r)
        #     plt.show()
