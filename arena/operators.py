...
class LogNormalPolicy(mx.operator.CustomOp):
    def __init__(self, deterministic=False, implicit_backward=True):
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        grad_mu = in_grad[0]
        grad_var = in_grad[1]
        self.assign(grad_mu, req[0], - (action - mean) * score.reshape((score.shape[0], 1)) / var)
...
class LogNormalPolicyProp(mx.operator.CustomOpProp):
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return LogNormalPolicy(deterministic=self.deterministic, implicit_backward=self.implicit_backward)
