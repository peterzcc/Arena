...
'''
Name: LogNormalNDPolicy
Usage: This OP outputs actions generated by a policy with normal distribution.
       The loss function for backward operation is set to be \sum_i - log(N(a_i|m_i, v_i)) * R_i
'''


class LogNormalNDPolicy(mx.operator.NDArrayOp):
    def __init__(self, deterministic=False):
        super(LogNormalNDPolicy, self).__init__(need_top_grad=False)
        self.deterministic = deterministic
    ...
    def forward(self, in_data, out_data):
        ...
        if 1 == len(var.shape):
            if self.deterministic:
                ...
                out_data[0][:] = nd.sqrt(var.reshape((var.shape[0], 1))) \
                                 * mx.random.normal(mean=0, stdvar=1, shape=mean.shape,
                                                       ctx=mean.context) + mean
        else:
            if self.deterministic:
                out_data[0][:] = mean
            else:
                out_data[0][:] = nd.sqrt(var) * mx.random.normal(mean=0, stdvar=1,
                                                                 shape=mean.shape,
                                                                 ctx=mean.context) + mean
...
'''
Name: SmoothL1RegressionLoss
Usage: This OP outputs the gradient of smooth l1 regression and
'''


class SmoothL1RegressionLoss(mx.operator.NumpyOp):
    def __init__(self, grad_scale=1.0):
        super(SmoothL1RegressionLoss, self).__init__(need_top_grad=False)
    ...
    def infer_shape(self, in_shape):
        ...
        loss_shape = (in_shape[0][0],)
        return [pred_shape, truth_shape], [loss_shape]

    def forward(self, in_data, out_data):
        x = in_data[0]
        truth = in_data[1]
        loss = out_data[0]
        flag = (numpy.abs(x - truth) < 1).astype(numpy.float32)
        print x.shape, truth.shape
        loss[:] = (0.5 * numpy.square(x - truth) * flag + (numpy.abs(x) - 0.5) * (1.0 - flag)).sum(axis=1)
