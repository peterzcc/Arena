...
class LogNormalPolicy(mx.operator.CustomOp):
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        grad_mu = in_grad[0]
        grad_var = in_grad[1]
        self.assign(grad_mu, req[0], - (action - mean) * score.reshape((score.shape[0], 1)) / var / score.shape[0])
        self.assign(grad_var, req[1], (0.5 / var - nd.square(action - mean) / nd.square(var) / 2) * score.reshape((score.shape[0], 1)) / score.shape[0] - numpy.float32(self.entropy_regularization) / var / 2)
...
class LogNormalPolicyProp(mx.operator.CustomOpProp):
    ...
    def infer_shape(self, in_shape):
        ...
        if self.implicit_backward == True:
            ...
            return [mean_shape, var_shape, score_shape, backward_action_shape], [output_shape, mean_shape, var_shape], []
