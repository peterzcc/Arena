...
if __name__ == '__main__':
    ...
    if dataset == "assist2009_old":
        ...
        parser.add_argument('--q_embed_dim', type=int, default=20, help='question embedding dimensions')
        parser.add_argument('--q_state_dim', type=int, default=10, help='hidden states of question')
        parser.add_argument('--qa_embed_dim', type=int, default=50, help='answer and question embedding dimensions')
        parser.add_argument('--qa_state_dim', type=int, default=25, help='hidden states of answer and question')

        parser.add_argument('--memory_size', type=int, default=20, help='memory size')
        parser.add_argument('--memory_key_state_dim', type=int, default=10, help='the key part of memory state dimension')
        parser.add_argument('--memory_value_state_dim', type=int, default=25, help='the value part of memory state dimension')
        ...
        parser.add_argument('--n_question', type=int, default=101, help='the number of unique questions in the dataset')
        parser.add_argument('--seqlen', type=int, default=200, help='the allowed maximum length of a sequence')
        ...
        parser.add_argument('--n_question', type=int, default=437, help='the number of unique questions in the dataset')
        parser.add_argument('--seqlen', type=int, default=200, help='the allowed maximum length of a sequence')
    ...
    file_name = 'key_value_one_' + 'batch' + str(params.batch_size) + 'qembed' + str(params.q_embed_dim) + 'qaembed' + str(params.qa_embed_dim) + \
    ...
    print train_q_data[1, :]
    print train_q_data[3, :]
    print train_q_data[5, :]
    print train_q_data[35, :]
    print train_q_data[55, :]
    print train_q_data[65, :]
    ...
    # python main.py --gpus 0 --k_smallest 5 --gamma 0.9 --init_std 0.05 --init_lr 0.1 --momentum 0.9 --maxgradnorm 50 --test True --show False --vis True
        ...
        epoch_file_name = os.path.join("key_value_one_batch64qembed100qaembed200qdim50qadim100msize50mkdim50mvdim100h1std0.5lr0.5mmt0.9gn50.0", "76_0.804523030177")
        load_path = os.path.join('model', params.load, epoch_file_name)
        net.load_params(name="KVMN_KT", dir_path=load_path)
        if params.vis:
            all_index_q_data = dat.generate_all_index_data(params.batch_size)
            print all_index_q_data
            _, _, _ = test(net, params, all_index_q_data, all_index_q_data, label='Test')
        else:
