...
def vis_kt(params, idx, pred, target, control_state, read_content, read_focus, write_focus):
    ...
    pred_3d = pred.reshape((params.seqlen, params.batch_size, params.n_question))
    for batch_iter in range(params.batch_size):
        ...
        vis_pred_all, vis_pred_one_hot, vis_target_one_hot = vis_matrix(params, pred_3d, target, batch_iter)
        ...
        plt.imshow(vis_pred_one_hot.T)
        a.set_title('vis_pred_one_hot')
        plt.xlabel(xlabel_name)
        plt.ylabel("number of exercises")
        plt.colorbar(orientation='horizontal')
        # PLT2Vis.display(data=vis_target_one_hot.T, x_label="sequence length", y_label="n_q", win_name="target_one_hot", scale=0.5)

        a = fig.add_subplot(2, 4, 3)
        plt.imshow(vis_target_one_hot.T)
        a.set_title('vis_target_one_hot')
        plt.xlabel(xlabel_name)
        plt.ylabel("number of exercises")
        plt.colorbar(orientation='horizontal')
...
def vis_weight(params, idx, target, read_focus, write_focus):
    for batch_iter in range(params.batch_size):
        ...
        a.set_title('read_focus'+str(seq_len))
        plt.xlabel(xlabel_name)
        plt.ylabel("memory size")
        fig.colorbar(ma)
        ...
        b.set_title('write_focus'+str(seq_len))
        plt.xlabel(xlabel_name)
        plt.ylabel("memory size")
        fig.colorbar(mb)
        plt.show()