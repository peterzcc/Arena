    ...
    def submit(self):
        def sge_submit(nworker, nserver, pass_envs):
            ...
            for i in range(nworker):
                submit_worker(node=workerq_l[i % len(workerq_l)], pass_envs=pass_envs, args=self.args)
            for i in range(nserver):
                submit_server(node=serverq_l[i % len(serverq_l)], pass_envs=pass_envs, args=self.args)
...
def main():
    ...
    parser.add_argument('-serverq', '--server-queue', default='all.q@client108,all.q@client109', type=str,
                        help = 'the queue we want to submit our server jobs to')
    parser.add_argument('-workerq', '--worker-queue', default='all.q@client110,all.q@client111', type=str,
    ...
    if args.jobname == 'auto':
        args.jobname = ('dmlc%d.%d' % (args.num_workers, args.num_servers)) + args.command[0].split('/')[-1];
    if args.logdir == 'auto':
