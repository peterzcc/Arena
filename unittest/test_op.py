...
def test_roi_wrapping():
    def calc_gt(data, rois, out_shape, explicit_batch, antialiasing, sampler_typ="bilinear"):
        ...
        return ret
    def ele_test_forward_roiwrapping(data_shape, out_shape, explicit_batch, antialiasing,
        ...
        sym = mx.sym.ROIWrapping(data=data, rois=rois, pooled_size=out_shape,
        ...
        outputs = exe.forward(is_train=True, data=data_npy, rois=rois_npy)
        assert reldiff(outputs[0].asnumpy(), gt) < 1E-6, "diff=%s, out=%s, gt=%s"\
                                                         %(str(outputs[0].asnumpy() - gt),
                                                           str(outputs[0].asnumpy()),
                                                           str(gt))
    def ele_test_backward_roiwrapping(data_shape, out_shape, explicit_batch, antialiasing,
                                      spatial_scale=1.0, sampler_typ="bilinear", ctx=mx.gpu()):
        data = mx.sym.Variable('data')
        rois = mx.sym.Variable('rois')
        sym = mx.sym.ROIWrapping(data=data, rois=rois, pooled_size=out_shape,
        ...
        check_numeric_gradient(sym, location={'data': data_npy, 'rois': rois_npy},
                               grad_nodes={'data': 'write', 'rois':'write'}, numeric_eps=1E-2,
                               check_eps=0.05,
                               ctx=ctx)
        check_numeric_gradient(sym, location={'data': data_npy, 'rois': rois_npy},
                               grad_nodes={'data': 'add', 'rois': 'add'}, numeric_eps=1E-2,
                               check_eps=0.05,
                               ctx=ctx)
    for ctx in [mx.cpu(), mx.gpu()]:
        for spatial_scale in [0.1, 0.3, 1.0]:
            ele_test_forward_roiwrapping(data_shape=(1, 1, 2, 2), out_shape=(3, 3), explicit_batch=True,
                                         spatial_scale=spatial_scale, antialiasing=False, ctx=ctx)
